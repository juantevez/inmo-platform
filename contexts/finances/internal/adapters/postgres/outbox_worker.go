...
func (w *OutboxWorker) Start(ctx context.Context, interval time.Duration) {
	...
	log.Printf("[FINANCES OUTBOX] Worker iniciado. Escaneando cada %v...\n", interval)

	for {
		select {
		case <-ctx.Done():
			log.Println("[FINANCES OUTBOX] Deteniendo el worker de Finanzas...")
			return
		case <-ticker.C:
			if err := w.processEvents(ctx); err != nil {
				log.Printf("[FINANCES OUTBOX ERROR] %v\n", err)
			}
		}
	}
}
...
func (w *OutboxWorker) processEvents(ctx context.Context) error {
	...
	log.Printf("[FINANCES OUTBOX] Encontrados %d eventos para enviar a NATS...\n", len(pending))

	for _, ev := range pending {
		if err := w.publisher.Publish(ctx, ev.subject, ev.payload); err != nil {
			log.Printf("[FINANCES OUTBOX WARN] Error al publicar evento %s: %v\n", ev.id, err)
			return err
		}

		updateQuery := `UPDATE finances_outbox_events SET status = 'PROCESSED', processed_at = CURRENT_TIMESTAMP WHERE id = $1;`
